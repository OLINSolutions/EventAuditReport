...
def process_events(args):
    ear_logger.get_logger().info('Processing Events only: Range start=%s, Range end=%s', args.start, args.end)
    event_processor.get_events(False)
    return

def process_all(args):
    ear_logger.get_logger().info('Processing Events and Notifications: Range start=%s, Range end=%s', args.start, args.end)
...
def process_command_line(argv=None, progDoc=''):
    ...
    #global config, eventRangeStart, eventRangeEnd, \
    #       xmod_url, authUser, authPassword, out_directory, event_filename, \
    #       notifs_filename, basicAuth, dirSep
...
''' % (program_shortdesc, program_user_name, str(config.date), 
    ...
    try:
        ...
        parser.add_argument("-v", dest="verbose", 
                            ...
                            help="set verbosity level.  Each occurrence of v increases the logging level.  By default it is ERRORs only, a single v (-v) means add WARNING logging, a double v (-vv) means add INFO logging, and a tripple v (-vvv) means add DEBUG logging [default: %(default)s]")
        parser.add_argument("-c", "--console", dest="noisy", 
                            action='store_true',
                            help="If specified, will echo all log output to the console at the requested verbosity based on the -v option")
        ...
        event_parser.add_argument('start',
                            help="Specify the event range start date/time in ISO 8601 format: yyyy-MM-dd'T'HH:mm:ssZ (e.g. 2017-01-25T10:45:48.011)")
        event_parser.add_argument('end',
                            help="Specify the event range end date/time in ISO 8601 format: yyyy-MM-dd'T'HH:mm:ssZ (e.g. 2017-01-26T10:45:48.011)")
        ...
        all_parser.add_argument('start',
                            help="Specify the event and notification range start date/time in ISO 8601 format: yyyy-MM-dd'T'HH:mm:ssZ (e.g. 2017-01-25T10:45:48.011)")
        all_parser.add_argument('end',
                            help="Specify the event and notification range end date/time in ISO 8601 format: yyyy-MM-dd'T'HH:mm:ssZ (e.g. 2017-01-26T10:45:48.011)")        
        ...
        try:
            ...
            raise(_CLIError(config.ERR_CLI_MISSING_DEFAULTS_MSG + args.defaults_filename, 
        ...
        logger.info('After parser.parse_args(), command_name=%s', args.command_name)
        ...
        if (config.events_filename is None):
            ...
            logger.info ('Events output filename is: %s', config.events_filename)
        if (config.notifs_filename is None):
            ...
            logger.info ('Notifications output filename is: %s', config.notifs_filename)
        
        # Validate the format for date and times is correct
        if (not __validate_date(config.event_range_start)):
            raise(_CLIError(config.ERR_CLI_INVALID_START_DATE_MSG%(config.event_range_start),
                            config.ERR_CLI_INVALID_START_DATE_CODE))
        if (not __validate_date(config.event_range_end)):
            raise(_CLIError(config.ERR_CLI_INVALID_START_DATE_MSG%(config.event_range_end),
                            config.ERR_CLI_INVALID_START_DATE_CODE))
